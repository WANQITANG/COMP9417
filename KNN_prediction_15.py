...
def judge_max_label(dist_label):
    ...
    for i in range(len(dist_label)):
        total_price+=float(dist_label[i][-1])
    pre_price=total_price/len(dist_label)
    return pre_price


def KNN_train(K,train_data,test_data,para):#weight 还没用，以后用
    # predit_list=[]# store the distacen and the {b,g}
    weighted_list=[]
    K_dist_label=caculate_distance(K,test_data,train_data)
    ##前K个距离，和他的label
    predict_price=judge_max_label(K_dist_label)
    print("predict:  ",predict_price)
...
if __name__ =='__main__':  
    ...
    data=np.delete(data,[1,2,3,4,5,6,7,13,14,16],axis=1)
    ...
    ####14+1个feature，1个label
    column_number_guiyi=range(15)
    ...
    for K in range(1,50):
        ...
        for i in range(len(data)):
            ...
            # weight =1
            para=50
            predit_lable=KNN_train(K,train_data,test_data,para)
